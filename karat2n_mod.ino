...
struct var {
  ...
  byte maxfreq = 100; // 100KHz Максимальный предел частоты
  ...
  if (newPosition != oldPosition) {
    switch (menu) {
      ...
      case 6: //Настройка minfreq
        if (newPosition > oldPosition && varinfo.minfreq <= 200) varinfo.minfreq = varinfo.minfreq + 1;
      ...
      case 7: //Настройка maxfreq
        if (newPosition > oldPosition && varinfo.maxfreq <= 200) varinfo.maxfreq = varinfo.maxfreq + 1;
        ...
        if (varinfo.maxfreq > 200) varinfo.maxfreq = 200;
        break;

       /* case 7: //Настройка maxfreq
        if (newPosition > oldPosition && varinfo.maxfreq <= 2000) varinfo.maxfreq = varinfo.maxfreq + 1;
        if (newPosition < oldPosition && varinfo.maxfreq >= 10) varinfo.maxfreq = varinfo.maxfreq - 1;
        if (varinfo.maxfreq <= varinfo.minfreq) varinfo.maxfreq = varinfo.minfreq + 1;
        if (varinfo.maxfreq > 2000) varinfo.maxfreq = 2000;
        break;*/
